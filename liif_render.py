...
class Fourier_render(torch.nn.Module):
    ...
    def query_F(self, feat, coord, mul):
        ...
        emb_fn, out_dims = fourier_embed.get_embedder(multires=mul, if_embed=True)  # out_dims = C not 3C
        fourier_base = emb_fn(coord)  # N,bsize,3C
    ...
    def forward(self, img_feature, h=256, w=256, mul=8, cell_scale=1, bsize=30000):
        coord = make_coord((h, w)).to(img_feature.device)  # h*w,2
        cell = 2 / (h * cell_scale) / 2  # cell_scale>1 for SR
        N = img_feature.shape[0]
        pred = self.batched_predict(img_feature,
...
class Fourier_render_patch(torch.nn.Module):
    ...
    def query_F(self, feat, coord, mul):
        ...
        emb_fn, out_dims = fourier_embed.get_embedder(multires=mul, if_embed=True)  # out_dims = C not 3C
        fourier_base = emb_fn(rel_coord)  # N,bsize,3C
    ...
    def query_F(self, feat, coord, mul):
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                rel_coord[:, :, 0] *= feat.shape[-2]
                rel_coord[:, :, 1] *= feat.shape[-1]  # scale to -1,1
                fourier_base_tmp=emb_fn(rel_coord) # N,bsize,3C
