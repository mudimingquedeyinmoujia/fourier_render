...
class Embedder:
    ...
    def create_embedding_fn(self):
        ...
        for freq in range(1, max_freq + 1):
            ...
            embed_fns.append(lambda x, freq=freq: 1/freq*torch.sin(x[:, :, 0] * freq).unsqueeze(-1))
            embed_fns.append(lambda x, freq=freq: 1/freq*torch.cos(x[:, :, 0] * freq).unsqueeze(-1))
            embed_fns.append(lambda x, freq=freq: 1/freq*torch.sin(x[:, :, 1] * freq).unsqueeze(-1))
            embed_fns.append(lambda x, freq=freq: 1/freq*torch.cos(x[:, :, 1] * freq).unsqueeze(-1))

            for freq_tmp in range(1, freq):
                for p_fn in p_fns:
                    embed_fns.append(lambda x, p_fn=p_fn, freqx=freq, freqy=freq_tmp:
                                     (1/(freqx*freqy))*p_fn(x[:, :, 0] * freqx, x[:, :, 1] * freqy).unsqueeze(-1))
    ...
    def forward(self, img_lr, h, w, retall=False, bsize=30000):
        ...
        if retall:
            ...
            for i in range(len(feature_list)):
                imghr_rend_tmp = self.render(feature_list[i], h=h, w=w, sf=self.rend_split[i][0],
                                             ef=self.rend_split[i][1], bsize=bsize)
